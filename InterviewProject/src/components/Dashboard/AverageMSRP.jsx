...
const AverageMSRP = ({ angmsrp }) => {
  // State to track selected dataset ("new", "used", "cpo")
  const [selectedCondition, setSelectedCondition] = useState('new');

  // Ensure angmsrp and the selectedCondition are defined before accessing them
  const conditionData = angmsrp?.[selectedCondition] || {}; // Fallback to empty object if undefined
  ...
  const data = {
    ...
    datasets: [
      {
        label: `${selectedCondition.charAt(0).toUpperCase() + selectedCondition.slice(1)} Average MSRP in USD`,
        data: counts,
  ...
  const options = {
    ...
    plugins: {
      title: {
        display: true,
        text: `Average MSRP in USD (${selectedCondition.toUpperCase()})`
      },
      tooltip: {
        mode: 'index',
        intersect: false
  ...
  return (
    <div className='overflow-x-auto text-start bg-white my-8' style={{ width: '100%', height: '80%', margin: '0 auto', textAlign: 'center', overflowX: 'auto' }}>
      ...
      <div className="flex justify-center gap-4 mb-4 items-center py-8 px-6 text-start">
        <span className='text-xl font-semibold'>Average MSRP in USD</span>
        <Button
